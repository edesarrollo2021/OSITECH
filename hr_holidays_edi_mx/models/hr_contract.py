...
class Contract(models.Model):
    ...
    def _get_contract_work_entries_values(self, date_start, date_stop):
        ...
        if self.env.context.get('holiday_status_id') not in ['holidays','personal']:
            calendar = self.company_id.resource_calendar_leave_id
        ...
        leave_domain = [
            ('time_type', 'in', ['leave', 'holidays', 'inability', 'personal', 'other']),
        ...
        for interval in real_leaves:
            ...
            contract_vals += [dict([
                                       ('name', "%s%s" % (leave_entry_type.name + ": " if leave_entry_type else "", employee.complete_name)),
