...
class ImportImagesEmployees(models.TransientModel):
    _name = 'import.images.employees'
    _description = 'Import Images for employees'

    company_id = fields.Many2one('res.company', string="Company", required=True, default=lambda self: self.env.company)
    file_ids = fields.Many2many('ir.attachment', string='File excel', required=False)
    file_zip_ids = fields.Many2many('ir.attachment', string='.ZIP', relation='import_images_users_attachment', required=False)
    ...
    def import_file_document(self):
        ...
        for row in range(1, sheet.nrows):
            if sheet.cell_value(row, 0):
                ...
                employee_id = self.env['hr.employee'].search([('registration_number', '=', enrollment), ('company_id', '=', self.company_id.id)], limit=1)
                if not employee_id:
                    msg_not_found.append(_('Not found User for email %s in the row %s. \n') % (sheet.cell_value(row, 0), str(row + 1)))
        ...
        for data in list_vals:
            ...
            employee_id.write({'image_1920': data['datas']})
