...
class EmployeeReportsSalary(models.AbstractModel):
    _name = "affiliate.movements.salary"
    _inherit = "report.affiliate.movements"
    _description = "Affiliate Movements Salary"
    ...
    filter_variable = [
        {'name': _lt('Variable'), 'selected': False, 'id': 'variable'},
    ]
    ...
    def _init_filter_structure(self, options, previous_options=None):
        if self.filter_structure is None:
            return
        options['variable'] = filter_variable = [
            {'name': _lt('Variable'), 'selected': False, 'id': 'variable'},
        ]

    @api.model
    def _get_query_lines(self, options, offset=None, limit=None):
        variable = options.get('variable', [])[0]['selected'] or False
        ...
        query = """
                SELECT
                    ...
                    hr_employee_affiliate_movements.type_change_salary,
                    ...
                    calendar.hours_per_week                 AS hours_per_week,
                    calendar.hours_per_day                  AS hours_per_day,
                ...
                LEFT JOIN resource_calendar calendar        ON calendar.id = contract.resource_calendar_id
                ...
                WHERE hr_employee_affiliate_movements.type = '07' AND %s
            """ % where_clause
    ...
    def _get_aml_line(self, options, aml):
        salary_type = aml['salary_type']
        if aml.get('type_change_salary', False):
            salary_type = aml.get('type_change_salary', False)
        ...
        if options.get('sicoss', False):
            columns = [
                ...
                {'name': 'M', 'class': 'whitespace_print'},
                {'name': '030', 'class': 'whitespace_print'},
            ...
            if self._context.get('print_txt', False):
                columns = [
                    ...
                    {'name': aml['employee_last_name'].upper() if aml['employee_last_name'] else '',
                     'class': 'whitespace_print'},
                    {'name': aml['employee_m_last_name'].upper() if aml['employee_m_last_name'] else '',
                     'class': 'whitespace_print'},
                    {'name': aml['employee_name'].upper() if aml['employee_name'] else '', 'class': 'whitespace_print'},
                    {'name': aml['sbc'], 'class': 'number'},
                    {'name': salary_type, 'class': 'whitespace_print'},
    ...
    def print_xlsx(self, options):
        return {
                ...
                'data': {'model': 'affiliate.movements.salary',
    ...
    def _txt_export(self, options):
        ...
        for line in txt_data:
            ...
            data[5] = "{0:.2f}".format(columns[5]['name']).replace('.','').zfill(6) if columns[5]['name'] else '0'.zfill(6)
            data[6] = ' '.ljust(6)
            ...
            data[11] = ' '.ljust(5)
            data[12] = columns[10]['name'].ljust(2) if columns[10]['name'] else ' '.ljust(2)
            data[13] = columns[11]['name'].ljust(5) if columns[11]['name'] and len(columns[10]['name']) == 5 else '0'.zfill(5)
            data[14] = str(columns[12]['name'])[0:10].ljust(10) if columns[12]['name'] else ' '.ljust(10)
            data[15] = ' '.ljust(1)
            data[16] = columns[13]['name'].ljust(18) if columns[13]['name'] else ' '.ljust(18)
            data[17] = '9'.ljust(1)
    ...
    def get_csv(self, options):
        ...
        for line in txt_data:
            ...
            array[12] = str(columns[12]['name'])[:8] if columns[12]['name'] else '0'.zfill(8)
            array[13] = columns[13]['name']
            array[14] = columns[14]['name']
            array[15] = str(columns[15]['name'])[:8] if columns[15]['name'] else '0'.zfill(8)
