...
class Tools(object):
    ...
    def send_s3_xsd(self, url_xsd):  # pragma: no cover
        ...
        response = urlopen.urlopen(url_xsd)
        ...
        for original_url in urls:
            ...
            response = urlopen.urlopen(original_url)
            content = response.read()
    ...
    @retry(URLError, tries=3, delay=3, back_off=2)
    def _cache_it(url):
        with closing(urlopen.urlopen(url)) as f_url:
            content = f_url.read()
            rep = r'href=[\'"]?([^\'" >]+)'
